...
class CL_E10_EliteAirMouse {
  private:
    ...
    // - commTask가 _state mutex를 잡지 않도록 "최신 스냅샷"만 전달
    // - 버튼 diff는 commTask에서 항상 처리, 이동/휠은 updated==true일 때만 전송
    // ----------------------------------------------------
    ...
    // (NEW) 런타임 적용 경로 단일화용 snapshot
    // - 부분 변경(setDpiLevel/setPrecisionMode 등)도 항상 applyRuntimeE10()로 통일
    ...
    // (AB) Observability: task stack / loop timing
  ...
  public:
    ...
    bool applyRuntimeE10(const ST_C10_E10Config_t& p_e) {
        ...
        // (리팩토링) 적용된 런타임 값을 "단일 진실"로 유지하기 위해 snapshot 갱신
    ...
    bool setHardClickLock(bool p_enable) {
        // (리팩토링) config contract(hard_click_lock)도 단일 적용 경로로 통일
        ST_C10_E10Config_t v_e;
        _getE10RuntimeConfig(v_e);
    ...
    void getStatus(ST_E10_Status_t& p_out) {
        ...
        // task stack watermark(남은 words의 최소값)
  ...
  private:
    ...
    // 런타임 config 단일 소스
    // - 부분 변경(setDpiLevel/setPrecisionMode 등)도 항상 applyRuntimeE10()로 통일
    // - _cfgE10RuntimeValid가 false면, 현재 런타임을 snapshot하여 기준을 만든다
    // -----------------------
    ...
    void _fsmUpdate(bool p_btnScroll, bool p_btnModeLongToggle, float p_gyroAbs) {
        if (p_btnModeLongToggle) {
            ...
            if ((v_nowMs - _lastModeToggleMs) >= _modeToggleCooldownMs) {
                ...
                // (AB) 모드 전환 시 stuck 방지: 강제 릴리즈 + smoothing reset
    ...
    static void _sensorTask(void* p_pv) {
        ...
        for (;;) {
            ...
            if (v_dtMs > 16.0f) { // target 8ms 기준 과도한 지연
                v_m->_dtOverrunCount++;
            }

            // (AB) stack watermark (min remaining words)
    ...
    static void _commTask(void* p_pv) {
        ...
        for (;;) {
            ...
            if (v_conn && !v_prevConn) {
                // connect 시점에 최신 프레임의 버튼 상태로 동기화(가능하면)
