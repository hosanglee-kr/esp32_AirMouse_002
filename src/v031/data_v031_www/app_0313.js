...
let g_schema = null;
let g_cfgBaseline = null; // deep-clone of last loaded/saved config for patch diff
...
// recursive diff: returns patch object containing only changed keys (including nested objects)
function diffObjects(oldObj, newObj){
  ...
  const patch = {};
  let changed = false;
  const keys = new Set([...(oldObj && typeof oldObj==='object' ? Object.keys(oldObj) : []), ...Object.keys(newObj)]);
  for(const k of keys){
...
function validateBySchema(cfg){
  ...
  function checkNode(node, val, label){
    ...
    const n = Number(val);
    if(!Number.isFinite(n)) { errs.push(`${label}: not a number`); return; }
...
function renderDiag(u){
  ...
  if(listEl){
    ...
    listEl.innerHTML = "";
    const shown = events.slice().reverse().filter(ev => !f || String(ev.code||"").includes(f)).slice(0, 80);
...
function bindDiag(){
  ...
  if(f) f.addEventListener("input", refreshDiag);
}
...
function validateClient(cfg){
  // Schema-driven validation (fallback: none)
  const errs = validateBySchema(cfg);
  return errs;
}
...
async function cfgSave(){
  ...
  let savedCfg = null;
  // text is patch; refresh baseline by reloading from device
  await cfgLoad();
