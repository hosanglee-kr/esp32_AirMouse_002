// =======================================================
// File: src/v010/C10_Config_0320.h
// =======================================================
#pragma once
/*
...
class CL_C10_Config {
  ...
  public:
    ...
    bool saveAll(const ST_C10_WiFiConfig_t& p_wifi, const ST_C10_E10Config_t& p_e10) {
        ...
        // -------------------------------------------------
        // Persist 강화:
        //  1) 기존 CFG_PATH -> CFG_BAK (rename 우선, 실패 시 copy)
        //  2) CFG_TMP -> CFG_PATH (rename 우선, 실패 시 copy)
        //  3) 최종 CFG_PATH 검증 실패 시 bak로 자동 복구
        // -------------------------------------------------
        if (LittleFS.exists(C10_DEF::CFG_BAK)) (void)LittleFS.remove(C10_DEF::CFG_BAK);

        // (A) rotate current -> bak
        if (LittleFS.exists(C10_DEF::CFG_PATH)) {
            bool v_bakOk = LittleFS.rename(C10_DEF::CFG_PATH, C10_DEF::CFG_BAK);
            if (!v_bakOk) {
                v_bakOk = _copyFile(C10_DEF::CFG_PATH, C10_DEF::CFG_BAK);
            }
            if (!v_bakOk) {
                // 기존 파일 보존(실패 시 tmp만 정리)
                (void)LittleFS.remove(C10_DEF::CFG_TMP);
                return false;
            }
        }
    ...
    // -------------------------------------------------
    // Config ETag (FNV-1a 32bit)
    // - 웹 UI 캐시/갱신(변경 감지)용
    // - CFG_PATH가 없으면 defaults 기반으로 계산
    // -------------------------------------------------
