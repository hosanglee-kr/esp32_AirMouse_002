...
class CL_W10_WebConfig {
  ...
  private:
    ...
    // =====================================================
    String& bodyGet(AsyncWebServerRequest* req, size_t index){
        if(index == 0){
    ...
    void bodyFree(AsyncWebServerRequest* req){
    ...
    void handleDynamicStatic(AsyncWebServerRequest* req){
        ...
        if(v_uri == "/favicon.ico"){
            serveWwwStatic(req, "/www/favicon.ico");
            return;
        }
        if(v_uri == "/robots.txt"){
            serveWwwStatic(req, "/www/robots.txt");
            return;
        }
        ...
        if(v_uri.startsWith(G_W10_URI_WWW_PREFIX)){
            ...
            serveWwwStatic(req, v_uri.c_str());
            return;
        }
        ...
        if(v_uri.startsWith(G_W10_URI_JSON_PUBLIC_PREFIX)){
            ...
            servePublicJson(req, v_uri.c_str());
            return;
        }
    ...
    // -----------------------------------------------------
    void serveWwwStatic(AsyncWebServerRequest* req, const char* p_path){
        if(!p_path){
            req->send(404, "text/plain", "not found");
            return;
        }
    ...
    void apiControl(AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t index, size_t total){
        ...
        if(err){
            JsonDocument out; out["ok"]=false; out["err"]="bad_json";
    ...
    void apiPostPpt(AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t index, size_t total){
        ...
        if(map.isNull()){
            JsonDocument out; out["ok"]=false; out["err"]="no_map";
            sendJson(req, out, 400);
            return;
        }
