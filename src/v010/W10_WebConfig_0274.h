// =======================================================
// File: src/v010/W10_WebConfig_0274.h
// =======================================================

#pragma once
/*
 * ------------------------------------------------------
 * 소스명 : W10_WebConfig_0274.h
 * 모듈약어 : W10
 * 모듈명 : Web Config/Status/UI/OTA Server (Field Dashboard + PPT + WiFi/E10 Editor)
 * ------------------------------------------------------
 * 기능 요약
 *  - 자산(HTML/CSS/JS) gzip 서빙 + 캐시 정책
 *  - /api/status : 현장용 대시보드 데이터(health/anomaly/i2c/err/ota/boot/net)
 ...
 *  - /api/config/save : 저장(atomic) + apply
 *  - /api/config/apply : 저장 없이 런타임 apply-only
 *  - /api/config/export/import/rollback : 백업/복구
 *  - /api/control : 런타임 control(ppt_mode/dpi/precision_mode)
 *  - /api/ota : Web OTA 업로드 + /api/ota/status
 *  - /api/safeboot : SafeBoot 상태조회/해제
 *  - /api/factory_reset : Factory Reset + reboot
 * ------------------------------------------------------
 * [구현 규칙]
...
class CL_W10_WebConfig {
  private:
    ...
    inline static constexpr ST_W10_Asset_t s_assets[] = {
      { "/",               "/www/index_0274.html", "/www/index_0274.html.gz", "text/html", false },
      { "/www/",           "/www/index_0274.html", "/www/index_0274.html.gz", "text/html", false },
      { "/www/style_0274.css", "/www/style_0274.css", "/www/style_0274.css.gz", "text/css", true },
      { "/www/app_0274.js",    "/www/app_0274.js",    "/www/app_0274.js.gz",  "application/javascript", true },
    };

    // mods mask == modifier byte (1:1)
    struct ST_W10_Mod_t { const char* name; uint8_t mask; };
    inline static constexpr ST_W10_Mod_t s_mods[] = {
    ...
    inline static constexpr ST_W10_Consumer_t s_consumer[] = {
        {"None", 0x00000000},
    ...
    static void s_wifiEvent(WiFiEvent_t e, WiFiEventInfo_t info){
        (void)info;
        if(s_instance) s_instance->onWifiEvent_(e);
    }
  ...
  public:
    ...
    void begin(CL_C10_Config* p_cfg, bool (*p_applyFn)(void*), void* p_applyCtx){
        ...
        // FS는 main에서 mount했어도, 여기서도 보강(실패해도 진행)
        ...
        for(size_t i=0;i<sizeof(s_assets)/sizeof(s_assets[0]);i++){
            ...
            _svr.on(a.uri, HTTP_GET, [this,a](AsyncWebServerRequest* req){
                this->serveAsset_(req, a);
            });
        }
        ...
        _svr.onNotFound([](AsyncWebServerRequest* req){
  ...
  private:
    ...
    void serveAsset_(AsyncWebServerRequest* req, const ST_W10_Asset_t& a){
        bool useGz = false;
        const char* path = a.plain;

        if(a.gz && LittleFS.exists(a.gz) && acceptsGzip_(req)){
            useGz = true;
            path = a.gz;
        }else{
            if(!LittleFS.exists(a.plain)){
                req->send(404, "text/plain", "asset not found");
                return;
            }
        }
    ...
    // save: body(json) -> cfg.importJson -> applyFn
    ...
    // apply-only: current load -> patch -> validate -> runtime apply(E10 only)
    void apiConfigApply_(AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t index, size_t total){
        ...
        out["ok"]=ok;
        out["applied"]=applied;
        out["note"]="apply-only: not saved. WiFi is validated but not applied to WiFi runtime.";
        sendJson_(req, out, ok?200:400);
    }
    ...
    void apiSafeBootGet_(AsyncWebServerRequest* req){
        ...
        sendJson_(req, d, (bool)d["ok"] ? 200 : 500);
    }
