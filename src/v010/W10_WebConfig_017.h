...
/*
 * ------------------------------------------------------
 * 소스명 : W10_WebConfig_017.h
 * 모듈약어 : W10
 * 모듈명 : Web Config Server (Gzip Assets, Keycodes, Status, STA+mDNS)
 * ------------------------------------------------------
 * 기능 요약
 *  - 정적 리소스(.gz 자동) + 캐시 정책(immutable/ no-store)
 *  - /api/keycodes: HID Usage ID 기반 키코드 대량 제공(매크로 의존 최소화)
...
class CL_W10_WebConfig {
  private:
    ...
    struct ST_W10_Asset_t {
        const char* uri;
        const char* fs_path_plain; // ex: /www/app_017.js
        const char* fs_path_gz;    // ex: /www/app_017.js.gz
        const char* content_type;  // ex: application/javascript
  ...
  private:
    ...
    void serveAsset(AsyncWebServerRequest* p_req, const ST_W10_Asset_t& p_a) {
        ...
        // 캐시 정책
        if (p_a.cache_immutable) {
            v_res->addHeader("Cache-Control", "public, max-age=31536000, immutable");
        } else {
