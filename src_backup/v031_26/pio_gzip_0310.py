...
# -------------------------------------------------------
# [json/public 동기화]
# - 프론트(UI)에서 사용할 스키마/공개 설정을 LittleFS의 /json/public/* 로 제공
# - gzip 없이 원본 json만 복사
# -------------------------------------------------------

SRC_JSONPUB_DIR = os.path.join(env["PROJECT_DIR"], env["PROJECT_SRC_DIR"], "v031", "data_v031_json_public")
DST_JSONPUB_DIR = os.path.join(env["PROJECT_DATA_DIR"], "json", "public")
...
def clean_dst_jsonpub():
    # ---------------------------------------------------
    # [DST 클린]
    # - data/json/public 아래에 남아있는 예전 파일이
    #   다음 빌드에서 그대로 포함되면 "구버전 json 서빙" 문제가 발생할 수 있음.
    # - 따라서 buildfs 직전에 public 폴더를 삭제하고,
    #   SRC 기준으로만 다시 구성한다.
    # ---------------------------------------------------
    if os.path.isdir(DST_JSONPUB_DIR):
        shutil.rmtree(DST_JSONPUB_DIR, ignore_errors=True)
...
def before_buildfs(source, target, env):
    ...
    print("[JSONPUB] Clean start")
    clean_dst_jsonpub()
    print("[JSONPUB] Clean done")

    print("[WWW] Sync+Gzip start")
    sync_www()
    print("[WWW] Sync+Gzip done")

    print("[JSONPUB] Sync start")
    sync_jsonpub()
    print("[JSONPUB] Sync done")
