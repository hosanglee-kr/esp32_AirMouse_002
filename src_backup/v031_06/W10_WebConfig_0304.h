...
class CL_W10_WebConfig {
  private:
    ...
    static constexpr uint8_t G_W10_BODY_SLOTS = 2;
...
bool _collectBodyOrReply(AsyncWebServerRequest* req,
    ...
    if (!v_slot) {
        JsonDocument out;
        out["ok"]  = false;
        out["err"] = "no_body_slot";
        _sendJson(req, out, 503);
        return false;
    }
...
void _apiConfigSaveImportCommon(AsyncWebServerRequest* req,
    ...
    out["ok"]      = v_ok;
    out["saved"]   = v_saved;
    out["applied"] = v_applied;
    out["note"]    = (p_note ? p_note : "");
    _sendJson(req, out, v_ok ? 200 : 400);
}
...
void _fillE10Status(JsonObject e, ST_W10_E10If_t* e10if) {
    ...
    void _apiStatus(AsyncWebServerRequest* req) {
        ...
        if (_cfg) {
            ...
            // 정책/배너 노출
            JsonObject pol = d["policy"].to<JsonObject>();
...
void apiConfigApply(AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t index, size_t total) {
    ...
    out["ok"]      = v_ok;
    out["applied"] = v_applied;
    out["note"]    = "apply-only: not saved. WiFi fields are ignored (E10 only).";
    _sendJson(req, out, v_ok ? 200 : 400);
}
...
if (_isSafeMode() && !_isApiAllowedInSafeMode(req->url().c_str())) {
    JsonDocument out;
    out["ok"] = false;
    out["err"] = "safe_mode_blocked";
    _sendJson(req, out, 403);
    return;
}
String v_body;
if (!_collectBodyOrReply(req, data, len, index, total, v_body)) return;
...
if (!_isSafeMode()) {
    ...
    if (e10if && e10if->getStatus) {
        ...
        if (e10if->getStatus(e10if->ctx, &s)) {
            if (s.ota_guard) {
                if (index == 0) {
                    req->send(403, "text/plain", "ota_guard_blocked");
                }
                return;
            }
        }
