// ======================================================
// File: src/v001/E10_EliteAirMouse_010.h
// ======================================================
#pragma once
/*
 * ------------------------------------------------------
...
class CL_E10_EliteAirMouse {
  private:
    ...
    static constexpr float G_E10_SCROLL_CURSOR_DAMP = 0.25f;

    // 캘리브레이션
    static constexpr uint32_t G_E10_CALIB_MS           = 1000;
    static constexpr float    G_E10_CALIB_STILL_TH_DEG = 3.0f;
    static constexpr uint32_t G_E10_CALIB_MIN_SAMPLES  = 20;
  ...
  private:
    ...
    void processGesturesDeg(float p_gzDegPerSec) {
        ...
        if (millis() - v_lastFlickMs < 600) return;
    ...
    void runGyroCalibration() {
        ...
        // fallback: 샘플이 너무 적으면 임계 완화 없이 전체 평균으로라도 계산
        if (v_cnt < G_E10_CALIB_MIN_SAMPLES) {
            Serial.printf("[E10] Gyro calib warning: samples too low=%u, using relaxed average\n", (unsigned)v_cnt);

            // 간단 완화: 다시 300ms 동안 모든 샘플 포함
            const uint32_t v_t1 = millis();
            v_cnt = 0; v_sumX = v_sumY = v_sumZ = 0.0;

            while (millis() - v_t1 < 300) {
                sensors_event_t v_a, v_g, v_temp;
                _mpu.getEvent(&v_a, &v_g, &v_temp);
                v_sumX += (v_g.gyro.x * RAD_TO_DEG);
                v_sumY += (v_g.gyro.y * RAD_TO_DEG);
                v_sumZ += (v_g.gyro.z * RAD_TO_DEG);
                v_cnt++;
                vTaskDelay(pdMS_TO_TICKS(5));
            }
        }
    ...
    static void sensorTask(void* p_pv) {
        ...
        for (;;) {
            ...
            if (v_scrollMode) {
                ...
                v_fx *= G_E10_SCROLL_CURSOR_DAMP;
                v_fy *= G_E10_SCROLL_CURSOR_DAMP;
            }
