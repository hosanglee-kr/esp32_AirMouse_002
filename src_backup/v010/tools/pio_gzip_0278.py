...
#   - svg/png/webp/ico/robots.txt: DST에 원본 복사( gzip 생성 X )
#   - 하위 폴더 구조 동일 유지
...
# 복사만 대상(바이너리/정적)
COPY_ONLY_EXTS = {".svg", ".png", ".webp", ".ico"}
    ...
    for root, _, files in os.walk(SRC_WWW_DIR):
        for fn in files:
            ...
            if ext in GZ_EXTS:
                ...
                # -> 원본(dst_path)은 굳이 생성하지 않는다
                continue

            # 2) 복사만 대상(이미지/ico/robots)
