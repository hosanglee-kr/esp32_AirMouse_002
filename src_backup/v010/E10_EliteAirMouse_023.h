// =======================================================
// File: src/v023/E10_EliteAirMouse_023.h
// =======================================================
#pragma once
/*
 * (기존 022 주석 동일 구조 유지, 생략)
 * 추가:
 *  - PPT Keymap v2: kb/consumer 전송
 *  - status anomaly: spike_count / consecutive_fail / consecutive_recover_fail
 */
#include <Arduino.h>
#include <Wire.h>
...
class CL_E10_EliteAirMouse {
  ...
  public:
    ...
    bool setDpiLevel(uint8_t p_level){ if(p_level<1)p_level=1; if(p_level>3)p_level=3; lock_(); _dpiLevel=(int)p_level; _engine.setDPI(_dpiLevel); unlock_(); return true; }
    bool setPrecisionMode(bool p_enable){ lock_(); _precisionMode = p_enable && _precisionEnable; _precSmX=0; _precSmY=0; unlock_(); return true; }
  ...
  private:
    ...
    void tapConsumerMask_(uint32_t p_mask, uint16_t p_ms=28){
        if(p_mask==0) return;
        // KeyboardDevice의 mediaKeyPress는 "mask" 방식 전제
    ...
    void sendPptKey2FromCfg_(const ST_C10_PptKey2_t& k){
    ...
    static void sensorTask(void* pv){
        ...
        for(;;){
            ...
            if(isnan(gx)||isnan(gy)||isnan(gz)){
                ...
                // 정상 값이면 연속실패를 일부 완화(완전 리셋보단 완만하게)
            ...
            if((after-before)==0){
                m->_errTaskOverrun++;
                if((m->_errTaskOverrun%10)==0) m->pushErr_(3,0);
            }
        }
    }
