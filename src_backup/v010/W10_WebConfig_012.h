...
/*
 * ------------------------------------------------------
 * 소스명 : W10_WebConfig_012.h
 * 모듈약어 : W10
 * 모듈명 : Web Config Server (Static + API, PPT KeyMap)
 * ------------------------------------------------------
 * 기능 요약
 *  - 011 기반 + /api/config에 ppt_keys 추가(저장/로드)
 * ------------------------------------------------------
 * [구현 규칙]
...
class CL_W10_WebConfig {
  private:
    ...
    static constexpr const char* G_W10_WWW_INDEX = "/www/index_011.html";
    static constexpr const char* G_W10_WWW_JS    = "/www/app_011.js";
    static constexpr const char* G_W10_WWW_CSS   = "/www/style_011.css";
    ...
    bool parseBodyToCfg(const String& p_body, ST_C10_E10Config_t& p_outCfg) {
        ...
        // 간단 범위 보호(0 허용: mod=0만 의미있음, key=0은 “비활성”)
    ...
    void setupRoutes() {
        _server.on("/", HTTP_GET, [&](AsyncWebServerRequest* p_req) {
            if (!LittleFS.exists(G_W10_WWW_INDEX)) { p_req->send(500, "text/plain", "Missing /www/index.html"); return; }
            p_req->send(LittleFS, G_W10_WWW_INDEX, "text/html");
        });

        _server.on("/www/app.js", HTTP_GET, [&](AsyncWebServerRequest* p_req) {
            if (!LittleFS.exists(G_W10_WWW_JS)) { p_req->send(404, "text/plain", "Missing /www/app.js"); return; }
            p_req->send(LittleFS, G_W10_WWW_JS, "application/javascript");
        });

        _server.on("/www/style.css", HTTP_GET, [&](AsyncWebServerRequest* p_req) {
            if (!LittleFS.exists(G_W10_WWW_CSS)) { p_req->send(404, "text/plain", "Missing /www/style.css"); return; }
        ...
        _server.on(
            ...
            [&](AsyncWebServerRequest* p_req) {
                ...
                if (!parseBodyToCfg(_rxBody, v_new)) { _rxBody = ""; p_req->send(400, "application/json", "{\"ok\":false,\"err\":\"json_parse\"}"); return; }
