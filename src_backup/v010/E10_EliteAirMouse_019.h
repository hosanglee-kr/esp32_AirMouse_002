...
/*
 * ------------------------------------------------------
 * 소스명 : E10_EliteAirMouse_019.h
 * 모듈약어 : E10
 * 모듈명 : AirMouse+Presenter (Composite HID, Status/Control v0.1.9)
 * ------------------------------------------------------
 * 기능 요약
 ...
 *  - [P0] MPU read 에러 카운터 반영 + task overrun 측정 개선
 *  - [정책] W10 mods(mask) = KeyboardDevice modifier byte(bitfield) 전송(호환 안정)
 * ------------------------------------------------------
 * [구현 규칙]
...
class CL_E10_EliteAirMouse {
  private:
    ...
    // [튜닝 TIP]
    // - Wire 핀(4,5)은 보드별로 다름. ESP32-S3-Zero는 보통 7/8 등.
    // - MPU6050 filter bandwidth, gyro range는 체감에 큰 영향.
  ...
  private:
    ...
    // ------------------------------------------------------
    // [정책] W10 /api/keycodes mods(mask) 와 동일 정책
    // - W10: mods[].mask = HID modifier bitfield(0x01..0x80)
    // - E10: KeyboardDevice::modifierKeyPress/Release()로 "modifier byte"에 직접 반영
    // - 즉, mask는 "0xE0 usage press"가 아니라 "modifier byte"가 정답(OS 호환 안정)
    // ------------------------------------------------------
    void applyModifierMask(uint8_t p_mask, bool p_press) {
        for (uint8_t v_bit = 0; v_bit < 8; v_bit++) {
            const uint8_t v_m = (uint8_t)(1u << v_bit);
            if ((p_mask & v_m) == 0) continue;
            if (p_press) _keyboard.modifierKeyPress(v_m);
            else _keyboard.modifierKeyRelease(v_m);
        }
    }
    ...
    void tapComboUsage(uint8_t p_modMask, uint8_t p_usage, uint16_t p_ms = 18) {
        if (p_usage == 0) return;
        if (p_modMask == 0) { tapKeyUsage(p_usage, p_ms); return; }

        applyModifierMask(p_modMask, true);
        _keyboard.keyPress(p_usage);
        vTaskDelay(pdMS_TO_TICKS(p_ms));
        _keyboard.keyRelease(p_usage);
        applyModifierMask(p_modMask, false);
    }
    ...
    static void sensorTask(void* p_pv) {
        ...
        for (;;) {
            // ----- P0: MPU read 에러 카운터 -----
            // Adafruit_MPU6050::getEvent는 void 리턴이라 "실패 여부"를 직접 받기 어렵습니다.
            // 대신 "dt 비정상/NaN" 및 온도/자이로 값 NaN 같은 이상값 감지로 에러 카운터를 누적합니다.
            // (하드 I2C 오류를 100% 잡진 못하지만, 현장 디버깅에 유의미)
            sensors_event_t v_a, v_g, v_temp;
            v_m->_mpu.getEvent(&v_a, &v_g, &v_temp);

            if (isnan(v_g.gyro.x) || isnan(v_g.gyro.y) || isnan(v_g.gyro.z) || isnan(v_temp.temperature)) {
                v_m->_errMpuRead++;
            ...
            if (!(v_dt > 0.0f && v_dt < 0.050f)) { // 0~50ms 범위 밖은 이상
                v_m->_errMpuRead++;
            }
            ...
            // ----- P0: task overrun 측정 개선 -----
            // 목표 주기(8ms) 대비 "얼마나 늦었는지"를 tick 기준으로 계산
            // v_lastWake는 delayUntil 내부에서 갱신되므로, 호출 전의 "예상 tick"과 비교
            TickType_t v_nowTick = xTaskGetTickCount();
            if (v_nowTick > v_lastWake) {
                v_m->_errTaskOverrun++; // 1 tick 이상 늦음
            }
