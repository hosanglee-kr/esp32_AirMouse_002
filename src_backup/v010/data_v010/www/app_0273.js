// =======================================================
// File: src/v0273/data_v0273/www/app_0273.js
// =======================================================
...
// boot banner
function bootBanner(boot){
  const el = $("bootBanner");
  el.classList.remove("boot","safe");
  const safe = !!(boot && boot.safe_mode);
  if(safe){
    el.classList.add("safe");
    el.textContent = "SAFE BOOT";
  }else{
...
async function refreshStatus(){
  try{
    ...
    const boot = s.boot || {};
    bootBanner(boot);
    $("sbState").textContent = `safe_mode: ${boot.safe_mode ? "ON":"OFF"}`;
    $("sbFail").textContent = `fail_count: ${boot.fail_count ?? "-"}`;
    $("sbPending").textContent = `pending: ${boot.pending ? "true":"false"}`;
    ...
    if(boot.safe_mode) addAlert(ul, "SAFE BOOT is active (WiFi forced AP).");
...
async function exitSafeBoot(){
  if(!confirm("Exit Safe Boot? (recommended to reboot after)")) return;
  const r = await jpost("/api/safeboot", { exit:true });
  $("sysMsg").textContent = pretty(r);
  await refreshStatus();
}

async function factoryReset(){
  const ok = confirm("Factory Reset will wipe config + boot state and reboot.\nProceed?");
  if(!ok) return;
...
function bind(){
  ...
  $("btnExitSafe").onclick = exitSafeBoot;
