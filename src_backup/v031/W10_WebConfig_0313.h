// =======================================================
// File: W10_WebConfig_0313.h
// =======================================================
#pragma once

/*
 * ------------------------------------------------------
 * 소스명 : W10_WebConfig_0313.h
 * 모듈약어 : W10
...
class CL_W10_WebConfig {
	...
	uint8_t _diagEvtHead = 0; // next write
	uint8_t _diagEvtCount = 0; // valid count
    ...
    // void begin(CL_C10_Config* p_cfg, bool (*p_applyFn)(void*), void* p_applyCtx) {
		...
		// API 라우팅 (전부 no-store)
		// ==============================
		_svr.on("/api/status", HTTP_GET, [this](AsyncWebServerRequest* req) {
			_apiStatus(req);
		});
		_svr.on("/api/diag", HTTP_GET, [this](AsyncWebServerRequest* req) {
			_apiDiag(req);
		});
		_svr.on("/api/diag/clear", HTTP_POST, [this](AsyncWebServerRequest* req) {
			_apiDiagClear(req);
		});
		_svr.on("/api/keycodes", HTTP_GET, [this](AsyncWebServerRequest* req) {
			apiKeycodes(req);
		});

		_svr.on("/api/config", HTTP_GET, [this](AsyncWebServerRequest* req) {
			apiGetConfig(req);
		});
		...
		_svr.on("/api/config/export", HTTP_GET, [this](AsyncWebServerRequest* req) {
			apiExport(req);
		});
		// (STEP12) alias for simpler client usage
		_svr.on("/api/export", HTTP_GET, [this](AsyncWebServerRequest* req) {
			apiExport(req);
		});
		...
		// PPT
		_svr.on("/api/ppt", HTTP_GET, [this](AsyncWebServerRequest* req) {
			apiGetPpt(req);
		});
		_svr.on(
			"/api/ppt",
		...
		// OTA
		_svr.on("/api/ota/status", HTTP_GET, [this](AsyncWebServerRequest* req) {
			apiOtaStatus(req);
		});
		...
		_svr.on("/api/safeboot", HTTP_GET, [this](AsyncWebServerRequest* req) {
			apiSafeBootGet(req);
		});
		_svr.on(
			"/api/safeboot",
		...
		// Reboot check (UI helper)
		_svr.on("/api/reboot/check", HTTP_GET, [this](AsyncWebServerRequest* req) {
			apiRebootCheck(req);
		});
	...
	void _apiStatus(AsyncWebServerRequest* req) {
		...
		// flat(legacy) - removable via ?flat=0 or ?compact=1
		...
		// Option: remove legacy flat keys and/or root duplicates
		if (!v_flat || v_compact) {
		...
		if (v_compact) {
			// Keep: policy, groups, (optional) config/boot/policy refs inside groups
			v_doc.remove("sys");
			v_doc.remove("mem");
			v_doc.remove("net");
			v_doc.remove("diag");
	...
	// =====================================================
	// /api/reboot/check (GET)
	// - UI helper: exposes current reboot requirement snapshot
	// =====================================================
