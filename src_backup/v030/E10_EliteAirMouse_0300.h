// =======================================================
// File: src/v0272/E10_EliteAirMouse_0300.h
// =======================================================
#pragma once
/*
 * (022 구조 유지 가정)
...
class CL_E10_EliteAirMouse {
  private:
    ...
    // RMS Welford
    uint32_t _gyroN    = 0;
    double   _gyroMean = 0, _gyroM2 = 0;
    uint32_t _curN    = 0;
    double   _curMean = 0, _curM2 = 0;
  ...
  public:
    ...
    void getStatus(ST_E10_Status_t& p_out) {
        ...
        for (uint8_t i = 0; i < _spikeCount; i++) {
            ...
            if (idx < 0) idx += 32;
            if (now - _spikes[idx].ts_ms <= 10000)
                sc++;
            else
                break;
        }
        ...
        p_out.health       = (score >= 820) ? (uint8_t)EN_E10_HEALTH_OK
                                            : ((score >= 620) ? (uint8_t)EN_E10_HEALTH_WARN : (uint8_t)EN_E10_HEALTH_DEGRADED);
  ...
  private:
    ...
    void applyE10ToRuntime_(const ST_C10_E10Config_t& e) {
        ...
        for (int i = 0; i < 3; i++) {
            _scaleBase[i] = e.scale_base[i];
            _accelGain[i] = e.accel_gain[i];
        }
        ...
        _ppt2_start = e.ppt2_start;
        _ppt2_exit  = e.ppt2_exit;
        _ppt2_next  = e.ppt2_next;
        _ppt2_prev  = e.ppt2_prev;
        _ppt2_black = e.ppt2_black;
        _ppt2_laser = e.ppt2_laser;
    ...
    void processGesturesDeg_(float gzDeg) {
        ...
        if (gzDeg > _gestureFlickDeg) {
            sendPptKey2FromCfg_(_ppt2_prev);
            last = millis();
        } else if (gzDeg < -_gestureFlickDeg) {
            sendPptKey2FromCfg_(_ppt2_next);
            last = millis();
        }
    }
    ...
    void runGyroCalibration_() {
        ...
        if (cnt > 0) {
            _gyroBiasX = (float)(sx / cnt);
            _gyroBiasY = (float)(sy / cnt);
            _gyroBiasZ = (float)(sz / cnt);
        }
        _gyroCalibDone = true;
    }

    static void mouseSend_(MouseDevice& ms, int8_t dx, int8_t dy, int8_t wheel) { ms.mouseMove(dx, dy, wheel, 0); }
    ...
    static void sensorTask(void* pv) {
        ...
        for (;;) {
            ...
            if (m->_fsm == EN_FSM_SCROLL) {
                ...
                int wheel = 0;
                if (gy > m->_wheelThDeg) {
                    float n = (gy - m->_wheelThDeg) / 120.0f;
                    if (n > 1) n = 1;
            ...
            if (m->_hid.isConnected()) {
                if (leftClick)
                    m->_mouse.mousePress(s_mouseBtnLeft);
                else
                    m->_mouse.mouseRelease(s_mouseBtnLeft);
            }
