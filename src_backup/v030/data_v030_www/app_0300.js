...
function wifiFingerprint(cfg){
  ...
  const m = w?.mdns || {};
  return [
    w.mode, s.ssid, s.pass, a.ssid, a.pass, m.host
  ].map(v=>String(v ?? "")).join("|");
}
...
function validateClient(cfg){
  ...
  // E10 numeric ranges (서버 validate와 동일한 범위로 맞춤)
  const e = cfg.e10 || {};
...
async function cfgLoad(){
  ...
  // WiFi fingerprint baseline 저장 (로드 직후는 변경 아님)
...
async function cfgSave(){
  ...
  const before = null;
  try{ before = currentJsonFromArea(); }catch(e){}
...
async function cfgImport(file){
  ...
  // 서버가 importJson(body)을 받는 구조라 /api/config/import로 그대로 POST
...
function bindUi(){
  ...
  // UI 변경 시 “JSON 반영” + “WiFi 변경 감지” (과도한 즉시 반영은 부담되니 input 이벤트로만)
  const watchIds = [
